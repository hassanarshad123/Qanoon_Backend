...
@router.get("/judge")
async def get_judge_profile(user: Annotated[SessionUser, Depends(get_current_user)]):
...
async def judge_tour_status(user: Annotated[SessionUser, Depends(get_current_user)]):
...
async def judge_tour_complete(user: Annotated[SessionUser, Depends(get_current_user)]):
...
@router.get("/lawyer")
async def get_lawyer_profile(user: Annotated[SessionUser, Depends(get_current_user)]):
...
async def get_lawyer_tour_state(user: Annotated[SessionUser, Depends(get_current_user)]):
...
async def lawyer_tour_welcome_shown(user: Annotated[SessionUser, Depends(get_current_user)]):
    return await profiles_repo.update_lawyer_tour_state(user.id, {"welcomeShown": True})


@router.post("/lawyer/tour-reset")
async def lawyer_tour_reset(user: Annotated[SessionUser, Depends(get_current_user)]):
